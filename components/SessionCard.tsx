...
const SessionCard: React.FC<SessionCardProps> = ({ session, tracks, onDoubleClick, onDragStart, isCompact = false }) => {
  ...
  if (isCompact) {
    return (
      <div
        ...
        className={`p-1.5 rounded-lg flex flex-col cursor-pointer transition-all duration-200 ease-in-out relative group min-h-12 hover:scale-102 ${statusColor[session.status]}`}
        style={{
        ...
        title={tooltipText}
      >
        <h4 className="font-medium text-xs leading-tight text-gray-900 dark:text-white truncate">{session.title}</h4>
        <p className="text-xs text-gray-400 dark:text-gray-400 leading-tight truncate">
  ...
  return (
    <div
      ...
      onDragStart={(e) => onDragStart(e, session)}
      onClick={() => onDoubleClick(session)}
      className={`h-full p-2 rounded-lg flex flex-col justify-between cursor-pointer transition-all duration-200 ease-in-out relative group hover:scale-102 ${statusColor[session.status]}`}
      style={{
    ...
    >
      <div>
        <h3 className="font-medium text-sm leading-tight text-gray-900 dark:text-white">{session.title}</h3>
        <p className="text-xs mt-1 text-gray-400 dark:text-gray-400 leading-tight">
          {session.speakers.map(s => s.name).join(', ')}
        </p>
      </div>
